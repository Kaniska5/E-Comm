...
async def refund_agent_node(state: AgentState) -> dict:
    prompt = """You are the Refund Processing Agent.
Your job is to check refund eligibility and process refunds.
First, check if the order is eligible.
If eligible, attempt to process the refund using the provided reason.
Do not invent order IDs. If a tool says human approval is required, inform the customer politely."""
